...
export abstract class BaseProvider implements ProviderInfo {
  ...
  protected resolveDockerUrl(baseUrl: string, serverEnv?: Record<string, string>): string {
    const isDocker = (typeof process !== 'undefined' && process?.env?.RUNNING_IN_DOCKER === 'true') || serverEnv?.RUNNING_IN_DOCKER === 'true';
  ...
  getProviderBaseUrlAndKey(options: {
    ...
    const apiKey =
      apiKeys?.[this.name] || serverEnv?.[apiTokenKey] || (typeof process !== 'undefined' ? process?.env?.[apiTokenKey] : undefined) || (manager as any).env?.[apiTokenKey];
